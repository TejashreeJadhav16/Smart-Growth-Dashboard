...
const fetchGoalDetails = async () => {
  try {
    const data = await $fetch(`http://localhost:5000/api/goals/${route.params.id}`);
    if (data) {
      form.value = {
        title: data.title,
        description: data.description || '',
        deadline: data.deadline,
        status_goal: data.status_goal || 'pending',
...
const handleUpdate = async () => {
  loading.value = true;
  try {
    await $fetch(`http://localhost:5000/api/goals/${route.params.id}`, {
      method: 'PUT',
      body: {
    ...
    alert("Failed to update goal. Check if your backend has a PUT route.");
  } finally {
    loading.value = false;
  }
};
